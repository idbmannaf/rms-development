...
class CompanyRfidEmployeeController extends Controller
{
    ...
    public function towerDetailsEntryExitHistoryLogCreateByChipIdPost(Request $request,Company $company)
    {
        $tower = Tower::where('chipid', $request->chipid)->first();
        $company = Company::where('id', $tower->company_id)->first();
        $request->validate([
            'select_employee' => 'required'
        ]);

        $rfidemployeeLog = RfidEmployeeLogs::where('chipid', $request->chipid)->where('door_closed_at', null)->latest()->first();
        if (!$rfidemployeeLog) {
            $newrfidemployeeLog = new RfidEmployeeLogs;
            $newrfidemployeeLog->chipid = $request->chipid;
            $newrfidemployeeLog->tower_id = $tower->id;
            $newrfidemployeeLog->rfid_employee_id = $request->select_employee;
            $newrfidemployeeLog->punch_at = now();
            $newrfidemployeeLog->door_open_at = now();
            $newrfidemployeeLog->lock_open_at = now();
            $newrfidemployeeLog->addedby_id = auth()->id();
            $newrfidemployeeLog->status = 1;
            $newrfidemployeeLog->is_manual = 1;
            $newrfidemployeeLog->save();

            return redirect()->route('company.rfid.towerWiseLockUnlockData', [$company,$request->chipid,'total'])->with('success', 'Log Created For Door Open');
        }

        return redirect()->back()->with('warning', 'Door already Open');
